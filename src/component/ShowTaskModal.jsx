...
const ShowTaskModal = () => {
  ...
  const handeldele = () => {
    dispatch(setShowTaskDelete(true));
    dispatch(setShowTaskModalMain(false));
  };
  const handelChangeCoulmn = (e) => {
    ...
    const upateColumn = selectBoard.columns.map((col) => {
      if (col.tasks.find((task) => task.title === selectTask.title)) {
        const updateTask = col.tasks.filter(
  ...
  return (
    <div className="bg-black/40 fixed top-0 left-0 h-screen w-full">
      <div
        ref={modalRef}
        className="bg-white dark:bg-dark-primary-100   dark:text-white  fixed w-2/5 max-h-max flex flex-col  -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 h-max p-8 rounded gap-4"
      >
        <div className="flex justify-between  dark:bg-dark-primary-10 ">
          <div className="text-lg font-bold  dark:bg-dark-primary-100 ">
            {selectTask.title}
          </div>
          ...
          {isOpenTask && (
            <div className=" absolute flex flex-col items-center bg-white w-32 h-15 z-50 top-12 -right-8 text-sm rounded shadow-2xl gap-2 p-2">
              <div className="text-gray-500" onClick={handelEdit}>
                Edit Task
              </div>
              <div className="text-red-400 " onClick={handeldele}>
                Delete Task
              </div>
            </div>
          )}
        </div>
        <span className="text-gray-400 text-xs font-normal  dark:bg-dark-primary-100 dark:text-white">
          {selectTask.description}
        </span>
        <p className="text-gray-400 text-xs">
          Subtasks {`(0 of ${selectTask.subtasks.length})`}{" "}
        </p>
        ...
        <label className="text-gray-400 text-xs  dark:bg-dark-primary-100 dark:text-white">
          current state
        </label>
        <select
          className="rounded border-2 border-gray-200 text-xs font-medium  p-2 hover:border-purpledo  dark:bg-dark-primary-100 "
          value={newColumn}
          onChange={handelChangeCoulmn}
        >
          <option>
