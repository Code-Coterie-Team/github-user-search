...
function ModalBoard() {
  ...
  return (
    <div className="bg-black/40 fixed h-full w-full top-0 left-0">
      <div
        ref={boradRef}
        className="fixed dark:bg-dark-primary-100 dark:text-white bg-white top-1/2 left-1/2  w-2/5  -translate-x-1/2 -translate-y-1/2 flex flex-col gap-6 p-9 rounded-xl"
      >
            ...
            {Array.isArray(newColumnName) &&
              newColumnName.map((col, index) => (
                <input
                  ...
                  key={index}
                  onChange={(e) => {
                    handelColumnChange(index, e.target.value);
                  }}
        ...
        <button
          className="text-purpledo p-2 rounded-xl bg-buttoncolor text-xs"
        ...
        <button
          className="bg-purpledo text-white text-xs rounded-xl p-2"
          onClick={handelSave}
        >
          Create New Board{" "}
        </button>
