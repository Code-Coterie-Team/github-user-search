...
function Sidebar() {
    ...
    //   side bar 
        <div className="grid grid-cols-6 w-full">
                        ...
                        <div key={index} className="text-center  "> 
                            ...
                            <div className= {`flex flex-col gap-4   h-full w-56 rounded border-gray-400 ${col.tasks.length === 0 ?'border-2 border-dashed border-gray-30 h-full':''}`} >
                                {col.tasks.map((task,taskIndex)=>(
                                    <div key={taskIndex} className=" w-full h-30 p-2 bg-white hover:opacity-20 cursor-pointer rounded shadow-2xl text-left " onClick={()=>openTaskDetail(task)}>
                                        <h4 className=" text-sm p-2">{task.title}</h4>
            ...
            {/* {modalDelete ? (<ModalDelete type={selectBoard.colum} />):undefined} */}
        </div>
